            ...
            all_data.append(
                {
                    ...
                    "round": round_i,
                    "armor": armor_bought,
                    "mattered_light": did_shield_matter(damage_instances, 125),
                    "mattered_no": did_shield_matter(damage_instances, 100),
                }
            )
...
def did_shield_matter(damage_events: list[int], threshold: int = 125):
    if sum(damage_events) >= threshold and sum(damage_events) < 150:
    ...
    for comb in permutations(damage_events):
        ...
        for damage in comb:
            curr_sum += damage
            if curr_sum >= threshold and curr_sum < 150:
